...
async def _resolve_bot_is_active(request: Request) -> bool | None:
    cached = read_is_active_from_redis()
    if cached is not None:
        return cached

    mongo = getattr(request.app.state, "mongodb_client", None)
    return await read_is_active_from_mongo(mongo)
...
async def toggle_bot_state(
    ...
    try:
        ...
        mongo = getattr(request.app.state, "mongodb_client", None)
        await save_is_active_to_mongo(mongo, new_state)
        bot.is_active = new_state
        write_is_active_to_redis(new_state)

...
import { Bot, FileText, Settings, TrendingUp, Users, MessageCircle } from "lucide-react";
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <CardHeader>
          <CardTitle className="text-xl flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 ml-auto">
              <span className={`text-sm font-semibold ${isBotActive ? 'text-[#da5b3e]' : 'text-gray-500'}`}>
          ...
          <CardDescription>
            Cuando el bot está inactivo, la burbuja sigue cargando, pero no responde preguntas.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className={`flex items-center justify-between p-4 rounded-xl border border-[#f0f0f0] shadow-[0_1px_3px_rgba(0,0,0,0.08)] bg-white`}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {quickActions.map((action, index) => (
            <Card key={index} className="group hover:shadow-xl shadow-sm transition-all duration-300 cursor-pointer border border-border/60 overflow-hidden">
              <CardHeader className="space-y-4">
                <div className={`w-12 h-12 rounded-lg ${action.gradient} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {stats.map((stat, index) => (
          <Card key={index} className="hover:shadow-xl transition-all duration-300 border border-border/60 shadow-sm">
